...
public:
    ...
    {
#ifdef WIN32
		std::cout << "nickname: " << nickname.data() << std::endl;
		strcpy_s(nickname_.data(), nickname.size() , nickname.data());
#else
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        boost::asio::io_service io_service;
		std::cout << "nickname:" << argv[1] << ",len:" << strlen(argv[1]) << std::endl;
		...
		memset(nickname.data(), '\0', nickname.size());

#ifdef WIN32
		std::cout << "nickname:" <<  argv[1] << ",len:" << strlen(argv[1]) << std::endl;
		strcpy_s(nickname.data(), nickname.size(), argv[1]);
