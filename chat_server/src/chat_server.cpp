...
{
	std::cout << "start getTimestamp" << std::endl;
	
#ifdef WIN32
	struct tm * now = new struct tm();  //tm结构指针
	time_t nowtm;                       //声明time_t类型变量
	time(&nowtm);
  
	localtime_s(now, &nowtm);  //获取系统日期和时间
...
#ifdef WIN32
		std::cout << " formatted_msg:" << formatted_msg.data() << "," << timestamp.length() << std::endl;
		std::cout << " timestamp:" << timestamp << std::endl;
		strcpy_s(formatted_msg.data(), formatted_msg.size(), timestamp.c_str());
		strcat_s(formatted_msg.data(), formatted_msg.size(), nickname.c_str());
		strcat_s(formatted_msg.data(), formatted_msg.size(), msg.data());
...
public:
    ...
    void start()
    {
		std::cout << "personInRoom start" << nickname_.data() << "%%" << std::endl;
...
private:
    ...
    {
		std::cout << "nicknameHandler start" << nickname_.data() << "%%"<< nickname_.size() << std::endl;
        if (strlen(nickname_.data()) <= MAX_NICKNAME - 2)
        {
#ifdef WIN32
            strcat_s(nickname_.data(), nickname_.size(), ": ");
#else
		...
		std::cout << "nickname_ disp" << nickname_.data() << "%%" << nickname_.size() << std::endl;
